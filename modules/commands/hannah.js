...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 50000) api.sendMessage("Bạn cần 50000 đô để xem ảnh nha",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 50000})
   var callback = () => api.sendMessage({body:`Ảnh hannaUwU\nSố Ảnh: ${link.length}\n-50000 đô nha bạn iu !`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
